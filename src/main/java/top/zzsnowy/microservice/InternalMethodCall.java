...
@Slf4j
@Data
public class InternalMethodCall extends Call{

    public InternalMethodCall(Sheet sheet, MSFileInfo msFileInfo) {
        ...
        for(int i = firstRowNum + 1; i <= lastRowNum; i ++){
            ...
            String weight = row.getCell(2).toString();
            if(!map.containsKey(source) || !map.containsKey(target)){
                log.info("计算内部纯方法调用，内部信息map中不存在的FEIGN类型跳过:" + source +
                        "->" + target);
                continue;
            }
